...
**[진행 방식]** 파일별로 어떤 변경을 할지 먼저 보여주고, 변경할지 여부를 나에게 물어봐. 내가 y라고 하면 적용하고, n이면 건너뛰어. 토큰 교체(색상/간격/radius)는 자동으로 해도 되지만, 컴포넌트 교체(<button>→<Button> 등 구조가 바뀌는 것)는 반드시 먼저 물어봐. DS 규칙 위반이 보이면(레이아웃 교체 포함) 확인 없이 즉시 수정한다. 범위 모호성은 이유가 안 된다.
...
- 섹션 간격 (페이지 레벨): gap-[var(--ds-section-gap)] — 카드 내부에 쓰면 과도한 간격 발생
- 내부 간격 (카드/컴포넌트 내부): gap-[var(--ds-internal-gap)] 또는 space-y-[var(--ds-internal-gap)]
- ❌ p-6, p-4, gap-4 등 구조적 패딩 직접 지정 금지 (gap-2 이하의 미세 아이콘 간격은 예외)
- ❌ 카드 내부에 \`space-y-[var(--ds-section-gap)]\` 금지 → \`space-y-[var(--ds-internal-gap)]\` 사용
...
- NavBar: <NavBar> <NavBarLogo> <NavBarNav> <NavBarNavLink> <NavBarActions> <NavBarMobileNav> <NavBarMobileNavLink>
...
import { BarChart, XAxis, YAxis, CartesianGrid, LineChart, Line, PieChart, Pie, Cell, AreaChart, Area, ResponsiveContainer } from "recharts"

const chartConfig = { revenue: { label: "매출", color: "var(--chart-1)" }, users: { label: "사용자", color: "var(--chart-2)" } }
<ChartContainer config={chartConfig} className="h-[300px] w-full">
  <BarChart data={data}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="name" /><YAxis /><ChartTooltip content={<ChartTooltipContent />} /><ChartBar dataKey="revenue" fill="var(--color-revenue)" /></BarChart>
</ChartContainer>
// ❌ <Bar> 직접 사용 금지 → 반드시 <ChartBar> 사용 (DS radius 토큰 자동 적용)
...
### 배경색 토큰 — 언제 무엇을 쓰는가 (용도 오용 = 즉시 수정 대상)
| 토큰 | 사용 상황 | 제약 |
|------|-----------|------|
| bg-background | 페이지 최상위 배경. body/main/NavBar에만 | 카드·패널에 절대 사용 금지 |
| bg-card | 카드·패널 배경 | **직접 className 사용 금지** — 반드시 \`<Card>\` 컴포넌트를 통해 적용. \`<Card>\`가 bg-card를 자동 적용함 |
| bg-muted | 비활성·보조 영역 (disabled 입력, 코드블록, 태그, 인라인 뱃지 배경) | 카드 전체 배경에 사용 금지 |
| bg-accent | hover 상태, 선택된 항목 배경 | 정적 배경에 사용 금지 — hover:/data-[state=active]: 등 상태 클래스에만 |
| bg-primary | 주요 액션 버튼 배경 | **버튼 외 사용 금지** — \`<Button variant="default">\`가 자동 적용 |
| bg-destructive | 삭제·위험 액션 버튼 배경 | **버튼 외 사용 금지** |
| bg-popover | 팝오버·드롭다운·툴팁 배경 | DS 오버레이 컴포넌트 내부에서만 |

- ❌ 카드처럼 생긴 영역에 \`bg-background\` + border 조합 금지 → 반드시 \`<Card>\` 사용
- ❌ raw \`<section>\` / \`<div>\` + border + rounded + padding = 카드 모양이라도 **\`<Card>\`로만** 구현
- ❌ 토큰 용도 오용 금지: \`bg-foreground\`, \`bg-muted-foreground\` — foreground 토큰은 텍스트 전용. 버튼을 검정으로 만들고 싶으면 DS Button에 variant="inverse" 추가 요청

### 반드시 사용
- ✅ 배경: 위 표 기준에 따라 용도에 맞는 토큰 선택
...
- ❌ 카드/UI 내부에서 \`<TypographyP>\` 금지 → \`<TypographySmall>\` 또는 \`<TypographyMuted>\` 사용. TypographyP는 문서 본문용(leading-7)이라 카드 안에서 과도한 여백 발생
- ❌ Typography 컴포넌트에 className으로 폰트 크기 오버라이드 금지 (\`<TypographyH2 className="text-xl">\` 등) → 원본 크기에 맞는 컴포넌트를 선택할 것: text-3xl급→H2, text-2xl급→H3, text-xl급→H4, text-base이하→TypographyLarge/TypographyP/TypographyMuted
...
- ❌ 커스텀 \`<header>\` 금지 → 반드시 \`<NavBar>\` + \`<NavBarLogo>\` + \`<NavBarNav>\` + \`<NavBarNavLink>\` + \`<NavBarActions>\` 사용 (NavBar 배경은 bg-background 단색)
- 사이드바 앱의 상단 바도 \`<NavBar sticky>\` 사용 — \`<SidebarTrigger />\`를 NavBar 첫 번째 자식으로 넣으면 됨. **별도 쉘 탑바 div / wrapper 절대 금지.** 이 규칙은 ESLint가 못 잡으므로 코드 작성 시 반드시 직접 준수:
  \`\`\`tsx
  <SidebarInset>
    <NavBar sticky>
      <SidebarTrigger />
...
**Sidebar 레이아웃 — 반드시 준수 (위반 시 레이아웃 깨짐):**
- ❌ \`<SidebarInset>\`에 \`w-full\` 금지 — \`flex-1\`과 충돌해 브라우저가 100vw로 렌더링, 사이드바와 겹침
- sticky header의 \`-mx-*\` 값은 반드시 부모 section의 \`px-*\` 값과 동일하게 — 불일치 시 \`overflow-x-hidden\`이 좌측 콘텐츠를 clip
- breakpoint는 콘텐츠 영역 너비 기준으로 설정 — sidebar가 있으면 실제 콘텐츠 영역이 viewport보다 ~256px 좁음. \`xl:grid-cols-5\` 같은 건 \`2xl:\`로 올릴 것
- 모바일에서 sidebar는 자동으로 Sheet으로 대체됨 — 모바일 topbar에 \`<SidebarTrigger />\`만 두면 됨, 별도 열림/닫힘 상태 관리 불필요

**기타 필수 규칙:**
- Button 크기는 반드시 size prop ("sm" | "default" | "lg" | "icon") — className으로 h-12, px-8 등 금지
- Input 높이는 기본값 유지 — className으로 높이 변경 금지
- 토스트/알림은 \`<Sonner>\` 사용. ❌ 커스텀 toast/snackbar state 금지 (\`const [toast, setToast] = useState(...)\`, \`snackbar.tsx\` 파일 생성 등) → \`import { toast } from "sonner"\`만 사용
...
- 배경색: 위 배경색 토큰 용도표에서 반드시 선택 — 나열만 보고 임의로 고르지 말 것
- 텍스트색: text-foreground, text-muted-foreground 등 시맨틱 색상만
- 테두리: border-border
- 둥글기: 아래 **커스텀 UI radius 선택 기준**에서 반드시 선택
...
### 커스텀 UI radius 선택 기준 (감으로 선택 금지 — 아래 기준표에서 반드시 선택)
| 용도 | 토큰 |
|------|------|
| 전체 화면 모달·시트·드로어 | rounded-[var(--ds-dialog-radius)] |
| 카드·패널·섹션·알럿 | rounded-[var(--ds-card-radius)] |
| 인풋·셀렉트·텍스트영역 | rounded-[var(--ds-input-radius)] |
| 드롭다운·메뉴·툴팁·뱃지·스켈레톤 | rounded-[var(--ds-element-radius)] |
| 버튼·토글·작은 인터랙티브 요소 | rounded-[var(--ds-button-radius)] |
| 아바타·프로필 이미지 (원형 강제) | rounded-full |

- ❌ rounded-md, rounded-lg, rounded-xl 직접 사용 금지 — 위 기준표에서 선택
- ❌ "대충 rounded-[var(--ds-card-radius)]" 금지 — 버튼·인풋 등 작은 요소에는 --ds-button-radius 사용
- 헷갈리면: 화면의 몇 %를 차지하는가? 큰 것(모달) → dialog, 중간(카드) → card, 입력 → input, 작은 UI → element/button
...
- 모바일 내비게이션: <NavBarMobileNav> 사용 (md 이하에서 Sheet으로 열림)
- 데스크톱 내비게이션: <NavBarNav> (md 이상에서 표시)
...
**폼 컴포넌트 — 버튼 variant 판단 기준 (반드시 아래 순서대로 판단):**

버튼 교체 시 원래 버튼의 **시각적 무게(visual weight)** 를 기준으로 variant를 결정한다.
**❌ 기존 버튼에 border 속성이 있다고 무조건 \`variant="outline"\` 금지 — 채워진 배경색이 있으면 \`variant="default"\` 또는 \`variant="secondary"\`**

| 원래 버튼의 시각적 특징 | 교체할 variant |
|-------------------------|---------------|
| 채워진 배경 (어두운 색: bg-gray-900, bg-black, bg-[#111], bg-foreground, bg-zinc-900 등) | \`variant="default"\` (bg-primary 자동 적용) |
| 채워진 배경 (밝은 주요색: bg-blue-600, bg-indigo-600, bg-[브랜드색]) | \`variant="default"\` |
| 채워진 배경 (보조색: bg-gray-200, bg-slate-100, bg-secondary 등) | \`variant="secondary"\` |
| 채워진 배경 (빨간색/위험: bg-red-600, bg-destructive) | \`variant="destructive"\` |
| 테두리만 있고 배경 없음 또는 bg-transparent (border + no fill) | \`variant="outline"\` |
| 배경도 테두리도 없음 (ghost, 텍스트만) | \`variant="ghost"\` |
| 밑줄 링크 스타일 | \`variant="link"\` |

- \`<button className="bg-blue/bg-primary...">\` → \`<Button>\` (= variant="default")
- \`<button className="bg-black/bg-gray-900/bg-foreground...">\` → \`<Button>\` (검정 버튼도 variant="default"로 교체 — bg-primary 자동 적용)
- \`<button className="border bg-transparent/배경없음...">\` → \`<Button variant="outline">\`
- \`<button className="bg-red/destructive...">\` → \`<Button variant="destructive">\`
- \`<button className="bg-secondary/bg-gray-100...">\` → \`<Button variant="secondary">\`
- \`<button className="ghost/투명/배경없음...">\` → \`<Button variant="ghost">\`
...
- \`<header>\` (전역 네비게이션 바) → \`<NavBar> <NavBarLogo> <NavBarNav> <NavBarNavLink> <NavBarActions> <NavBarMobileNav>\`
...
- ❌ 커스텀 탭 구현 금지 (Button + activeTab state + 조건부 스타일) → 무조건 \`<Tabs><TabsList><TabsTrigger><TabsContent>\`로 교체. CSS 셀렉터 기반 tabClass 변수도 전부 삭제. 탭 버튼에 \`<Button>\` 쓸 경우 반드시 variant="ghost" 지정, 미지정 시 bg-primary(파란색) 버튼으로 깨짐. TabsTrigger 안에 \`<Badge>\` 등 추가 요소를 넣을 수 있다.
- ❌ 탭/선택 UI에 \`rounded-full\` 금지 → pill형 탭은 \`<TabsList variant="pill">\`, 둥글기는 반드시 \`var(--ds-element-radius)\` 사용. \`rounded-full\`은 Avatar/Badge 전용.
...
2. **globals.css 확인**: 파일 맨 위에 아래 import가 있는지 확인하고, 없으면 즉시 추가:
   \`@import url("${tokensCssUrl}"); /* DesignSync tokens */\`
   이 import가 없으면 폰트 변수(--ds-font-sans 등)와 @font-face가 로드되지 않아 폰트가 적용되지 않는다.
3. 기존 코드의 모든 파일을 스캔하여 위 교체 패턴을 수행 — 토큰 교체는 자동으로, 컴포넌트 교체는 파일별로 먼저 보여주고 y/n 확인 후 적용
4. 이후 새로 작성하는 모든 코드도 위 규칙을 따를 것
