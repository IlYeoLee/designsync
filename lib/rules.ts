...
2. **설치 직후, 기존 코드의 모든 파일을 스캔하여 하드코딩된 색상(bg-blue-600, text-gray-500 등), 크기(h-9, rounded-xl 등), 패딩(p-6 등)을 위 토큰으로 즉시 전부 교체하라.** 유저가 별도로 요청하지 않아도 이 작업을 자동으로 수행한다.
3. 커스텀 HTML 요소도 가능하면 DesignSync 컴포넌트로 교체한다 (커스텀 button → <Button>, 커스텀 modal → <Dialog>, 커스텀 sidebar → <Sidebar> 등)
4. 이후 새로 작성하는 모든 코드도 위 규칙을 따를 것

## ${n + 5}. 디자인 업데이트 (웹에서 수정 후 반영)
유저가 "디자인 업데이트해" 또는 "디자인 싱크해"라고 하면 아래 명령어를 실행:
\`\`\`bash
curl -fsSL ${CDN}/api/sync${setupParam} | node
\`\`\`
이 명령어는 globals.css의 토큰만 최신으로 갱신한다 (컴포넌트 재설치 없음, 3초 소요).` : ""}`.trim();
}
