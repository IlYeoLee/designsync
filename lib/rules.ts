...
**단순 위반 → y/n 묻지 않고 자동 일괄 수정:**
- 하드코딩 색상 토큰 (bg-gray-*, text-white, border-gray-* 등)
- 하드코딩 radius / height / padding (rounded-lg, h-9, p-6, gap-4 등)
- 폰트 크기·굵기·줄간격 arbitrary 값 (text-[14px], font-black, leading-[1.5] 등)
- next/font, Google Fonts import
- 아이콘 라이브러리 교체 (import 경로만 변경)

**고난도 위반 → 아래 형식으로 보여주고 y/n 대기:**
...
고난도 해당 항목:
- \`<aside>\` → \`<Sidebar>\` + SidebarProvider 구조 전환
- 커스텀 탭 구현 → \`<Tabs>\` 전환
- 커스텀 모달 div → \`<Dialog>\` / \`<Sheet>\` 전환
- 커스텀 드롭다운 → \`<DropdownMenu>\` 전환
- SVG/canvas 차트 → recharts + \`<ChartContainer>\` 전환
- raw \`<h1>~<h6>\` 포함 대형 JSX 블록 교체
- \`<header>\` / \`<aside>\` 레이아웃 전체 재구성

- 내가 y → 즉시 수정 → **대기하지 말고 곧바로 다음 위반 스캔**
- 내가 n → 건너뛰고 → **대기하지 말고 곧바로 다음 위반 스캔**
- 모든 위반 처리 후 → **재검토 1회 강제**: "재검토 중..." 출력 후 전체 코드를 다시 스캔하여 빠진 위반이 있으면 이어서 처리, 없으면 "✅ 위반 없음 확인" 출력 후 종료

**[중간 보고 원칙]**
- 파일 10개 처리마다: `"📊 진행 중: X/Y 파일 완료 — 단순 N건 수정됨"` 출력
- 단순 위반 자동 수정 전체 완료 시: `"✅ 단순 위반 완료 (N건 수정) — 고난도 N건 진행합니다"` 출력 후 고난도 루프 시작

❌ 금지: 내가 "계속해" 라고 말하기를 기다리는 것 / 단순 위반을 고난도로 분류해서 불필요하게 y/n 요청하는 것
...
- 폰트 굵기: font-normal(400), font-medium(500), font-semibold(600), font-bold(700), font-extrabold(800) — 이 외 금지
- ❌ font-black(900), font-[700], font-[900] 등 임의 굵기 — **절대 금지** (너무 뚱뚱한 텍스트 방지)
- 줄 간격: leading-tight, leading-normal, leading-relaxed, leading-loose — 이 외 금지
- ❌ leading-[1.5], leading-[24px], leading-[1.8] 등 임의 line-height — **절대 금지**
...
- 아이콘 색상: 반드시 시맨틱 토큰 사용 — text-[color:var(--icon)] (기본), text-[color:var(--icon-muted)] (비활성), text-destructive (오류), text-muted-foreground (흐림)
- ❌ 금지: 아이콘에 text-red-500, text-gray-400 등 하드코딩 색상 클래스 — **절대 금지**
- ❌ 금지: 아이콘에 style={{ color: '...', width: '20px' }} 인라인 스타일 — **절대 금지**
...
- ❌ \`leading-[1.5]\`, \`leading-[24px]\` 등 arbitrary 줄 간격 — **절대 금지**
- ❌ \`font-black\`, \`font-[700]\`, \`font-[900]\` 등 arbitrary/극단적 굵기 — **절대 금지**
- ❌ 아이콘에 \`text-red-500\`, \`text-gray-400\` 등 하드코딩 색상 — **절대 금지**, 반드시 text-[color:var(--icon)] 등 시맨틱 사용
...
3. **[사전 체크]** 수정 시작 전, 전체 src/ 를 스캔하여 아래 형식으로 요약만 출력 (수정 금지):
   \`\`\`
   📋 사전 체크 결과
   ├─ raw HTML 요소: N건 (파일 목록)
   ├─ 하드코딩 색상: N건 (파일 목록)
   ├─ 하드코딩 radius/height/padding: N건 (파일 목록)
   ├─ 폰트 위반: N건 (파일 목록)
   └─ 기타: N건
   총 N건 — 수정을 시작할까요? (y/n)
   \`\`\`
4. 내가 y → **[자동 수정 + 고난도 루프]** 단순 위반은 일괄 자동 수정, 고난도 위반만 위 진행 방식대로 y/n — y든 n이든 즉시 다음 위반 처리 (내가 먼저 말하기를 기다리지 말 것)
5. 모든 위반 처리 후 **재검토 1회 강제**: "재검토 중..." 출력 → 전체 재스캔 → 남은 위반 있으면 루프 계속, 없으면 \`npx eslint src/\` 실행하여 에러 0 확인
6. 에러 0 확인 후 "✅ DesignSync 적용 완료" 출력
7. 이후 새로 작성하는 모든 코드도 위 규칙을 따를 것
